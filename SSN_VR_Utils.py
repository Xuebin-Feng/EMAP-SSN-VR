...
def build_network_from_raw(data, forced_ref_header=None):
    ...
    # Layout Init
    side = int(np.ceil(n_nodes_new ** (1/3)))
    base_box = (n_nodes_new ** (1/3)) * 2.5 + 5.0
    box_limit = base_box * getattr(cfg, 'BOX_SCALE', 1.0)
    x = np.linspace(-box_limit*0.5, box_limit*0.5, side)
    y = np.linspace(-box_limit*0.5, box_limit*0.5, side)
    z = np.linspace(-box_limit*0.5, box_limit*0.5, side)
    xv, yv, zv = np.meshgrid(x, y, z)
    pos = np.column_stack((xv.flatten(), yv.flatten(), zv.flatten()))[:n_nodes_new].astype(np.float32)

...
def get_colormap(scheme_name):
    ...
    try:
        if hasattr(mpl, 'colormaps'):
            return mpl.colormaps[scheme_name]
    except KeyError:
        pass
    
    # Try using cm.get_cmap
    try:
        return cm.get_cmap(scheme_name)
    except Exception:
        pass
...
def run(viewer, args):
    ...
    if np.sum(mask) == 0:
        Command_Engine.print_help(viewer, "No nodes matched the selection criteria.")
        return

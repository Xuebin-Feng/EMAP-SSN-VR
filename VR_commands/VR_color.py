...
def run(viewer, args):
    ...
    def push_assignment():
        ...
        if not current_expr and (current_color or current_scale):
            current_expr = '@_sele.txt@'
            
        if current_expr and (current_color or current_scale):
    ...
    if current_expr or current_color or current_scale: 
        push_assignment()
    ...
    for expr, color_str, scale_val in assignments:
        ...
        try:
            ...
            if count > 0:
                ...
                if scale_val: viewer.current_sizes[mask] = getattr(cfg, 'NODE_SIZE', 10.0) * scale_val
                ...
                print(f"No nodes matched expression: {expr}")
                
        except Exception as e:
            print(f"Error executing '{expr}': {e}")
            import traceback
            traceback.print_exc()

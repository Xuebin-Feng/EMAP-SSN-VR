...
SETTINGS_FILE = os.path.join("Cache_Files", "viewer_settings.json")
...
if __name__ == "__main__":
    ...
    class ConfigGUI(QMainWindow):
        ...
        def setup_tips(self):
            self.tip_db_keys = {
                "NODE_FASTA_FILE": "The sequence set or subset to be visualized in the SSN.",
                "MSA_FILE": "The multiple sequence alignment file (.fasta, _sparse.pkl) containing at least the sequences in the FASTA sequence set.",
                "INPUT_HDF5": "The network or E-value matrix file (.hdf5) containing the edge weights and metrics for the SSN.\nMust contain at least the sequences in the FASTA sequence set.",
                "ALIGNMENT_SCORE": "(For embedding-based SSNs only) Type of similarity score (global vs local) to use for SSN.",
                "NORM_MODE": "(For embedding-based SSNs only) Normalization mode for similarity scores.",
                "ALIGNMENT_REFERENCE": "The sequence header substring for identifying MSA reference sequence.",
                "SIMILARITY_THRESHOLD": "Threshold for filtering edges based on similarity scores; edges with scores below this threshold are not plotted in the SSN.",
                "TOP_EDGE_PERCENT": "Automatically calculates threshold based on percentage of total edges retained. \nOverrides Similarity Threshold if this value is defined.",
                "FILTER_MIN_OCCUPANCY": "Alignments with less occupancy than this percentage are filtered out during clustering.",
                "NODE_SIZE": "Visual rendering parameter. Average size of each network node point.",
                "EDGE_WIDTH": "Visual rendering parameter. Thickness of the edge connections drawn.",
                "EDGE_ALPHA": "Transparency of drawn edges. 0.0 is invisible, 1.0 is fully opaque.",
                "NEIGHBOR_COLOR": "Highlight color used when analyzing neighbor interactions locally.",
                "HOVER_COLOR": "Highlight color when hovering the mouse over a node.",
                "CONNECTED_NODE_COLOR": "Highlight color for the outer contour of nodes directly connected to selected nodes.",
                "EDGE_COLOR": "Color name or Hex code (#) for the edges connecting the nodes.",
                "NODE_BOUNDARY_COLOR": "Color name or Hex code (#) for the border/outline around nodes.",
                "NODE_BOUNDARY_WIDTH": "Visual rendering parameter. Thickness of the border/outline around nodes.",
                "SPRING_K": "Physics constant: Attractive force magnitude between connected node pairs.",
                "COULOMB_K": "Physics constant: Repulsive force magnitude between separated node pieces.",
                "COULOMB_CUTOFF": "Distance threshold past which repulsive physics drops off.",
                "DAMPING": "Frictional resistance applied to physics rendering to stabilize movement.",
                "DT": "Timestep size for each layout physics calculation iteration.",
                "MAX_STEPS": "Maximum iterations the simulation layout loop will run before halting.",
                "RMSD_THRESHOLD": "Simulation stopping criteria if spatial changes fall below this threshold.",
                "PERCENTAGE_DROP_THRESHOLD": "Stops the simulation if the RMSD moving average drops by less than this percentage over one full window. Set to 0 to disable.",
                "RMSD_WINDOW": "The number of steps over which the RMSD moving average is calculated.",
                "ENABLE_PROGRESSIVE_SIMULATION": "Gradually lowers the similarity threshold in stages for massive components. Turn OFF if the graph fails to converge.",
                "MSA_DIR": "Directory path loaded dynamically for MSA dropdown files.",
                "SAVED_LAYOUT_DIR": "Location where layout .pkl cache networks are generated and re-loaded from.",
                "PRINT_SAVE_DIR": "Folder for exporting final PDF/PNG image snapshots.",
                "FASTA_SPLIT_DIR": "Location where extracted sequence subset fasta files are saved.",
                "CLUSTER_LABEL_DIR": "Exports cluster metadata names per file.",
                "HEADER_LIST_DIR": "Saves lists of headers corresponding to particular layout filters.",
                "HISTORY_DIR": "Location where the viewer saves and loads your command history (.txt) for each network.",
                "LOGO_DIR": "Folder for exporting sequence logos generated by the logo command.",
                "TARGET_CACHE_FILE": "Selects a specific previously-saved layout iteration from the target folder to visualize.",
                "NEW_CACHE_NAME": "Specifies a custom filename when saving a new layout iteration (Requires 'New Layout Cache' to be selected)."
            }
        ...
        def update_live_validators(self):
            ...
            if hasattr(self, 'line_thresh') and hasattr(self, 'line_top'):
                ...
                self.line_thresh.setEnabled(not has_top_edge)
            ...
            from SSN_VR_Utils import simplify_node_label
            import re
        ...
        def create_visuals_tab(self):
            ...
            # 2. Colors Setup
            color_keys = ["NEIGHBOR_COLOR", "HOVER_COLOR", "CONNECTED_NODE_COLOR", "EDGE_COLOR", "NODE_BOUNDARY_COLOR"]
            self.visual_defaults = {
                "NODE_SIZE": 10, "EDGE_WIDTH": 1.0, "NODE_BOUNDARY_WIDTH": 0.5, "EDGE_ALPHA": 0.1,
                "NEIGHBOR_COLOR": "#4488ff", "HOVER_COLOR": "#ffaa00", "CONNECTED_NODE_COLOR": "#ff0000",
            ...
            # 3. Edge Downsampling Toggle Setup
            filt_container = QWidget()
            filt_container.setObjectName("wrapper")
            h_filt = QHBoxLayout(filt_container)
            h_filt.setContentsMargins(0, 0, 0, 0)
            h_filt.setSpacing(15)
            
            cb_filt = QPushButton()
            cb_filt.setCheckable(True)
            cb_filt.setFixedSize(60, 28)
            
            def switch_toggle_style_filt(checked, btn=cb_filt):
                if checked:
                    btn.setText("ON")
            ...
            cb_filt.toggled.connect(switch_toggle_style_filt)
            
            initial_filt_state = bool(globals().get("ENABLE_EDGE_FILTERING", True))
            cb_filt.setChecked(initial_filt_state)
            switch_toggle_style_filt(initial_filt_state)
            
            lbl_max = QLabel("Max Number of Edges:")
            spin_max_edges = NoScrollSpinBox()
            spin_max_edges.setRange(1000, 10000000)
            spin_max_edges.setSingleStep(50000)
            spin_max_edges.setFixedWidth(100)
            
            initial_max_edges = int(globals().get("MAX_RENDER_EDGES", 500000))
            spin_max_edges.setValue(initial_max_edges)
            
            # Connect toggle to setEnabled
            cb_filt.toggled.connect(spin_max_edges.setEnabled)
            spin_max_edges.setEnabled(initial_filt_state)
            
            h_filt.addWidget(cb_filt)
            h_filt.addWidget(lbl_max)
            h_filt.addWidget(spin_max_edges)
            h_filt.addStretch()
            
            lbl_filt = QLabel("Enable Edge Downsampling:")
            form_layout.addRow(lbl_filt, filt_container)
            self.labels["ENABLE_EDGE_FILTERING"] = lbl_filt
            self.inputs["ENABLE_EDGE_FILTERING"] = cb_filt
            self.inputs["MAX_RENDER_EDGES"] = spin_max_edges
        ...
        def create_physics_tab(self):
            ...
            # --- 6. Progressive Edge Annealing row (Show Score Histogram omitted) ---
            cb_row = QWidget()
            ...
            # Add a dummy QWidget to take the right column space so Progressive Edge Annealing stays 50% width
            right_empty = QWidget()
            cb_layout.addWidget(right_empty)
        ...
        def create_directories_tab(self):
            ...
            keys = ["FASTA_DIR", "MSA_DIR", "HDF5_DIR", "SAVED_LAYOUT_DIR", "PRINT_SAVE_DIR", 
        ...
        def save_and_run(self):
            if self.save_settings():
                ...
                subprocess.Popen([sys.executable, "SSN_VR_Viewer.py"], env=env)

...
def print_help():
    ...
    to the configured Cluster Label directory (Analysis_Results/Cluster_Labels).
...
def _alignment_records(aln):
    """Yield one SeqRecord per row of the loaded alignment.

    Alignment_Manager's sparse loaders build a SeqRecord inside __getitem__
    without ever importing Seq/SeqRecord, so indexing or iterating one raises
    NameError and every split below died before it began. Rows are therefore
    rebuilt here straight from the sparse matrix; a real Biopython alignment
    is iterated normally. commands/logo.py carries the same workaround.
    """
    ...
    headers = getattr(aln, 'headers', [])
    for row_index in range(matrix.shape[0]):
        row = matrix[row_index].toarray()[0]
        seq_str = "".join(
            int_to_aa.get(value, '-') if value != 0 else '-' for value in row)
        desc = headers[row_index] if row_index < len(headers) else f"row_{row_index}"
        yield SeqRecord(Seq(seq_str), id=desc.split()[0], description=desc)


def get_sequence_stats(records):
    lengths = []
...
def run(viewer, args):
    ...
    # Help is answered before the prerequisites. The terminal is the only place
    # this text can be read in VR, and withholding it until an MSA and a
    # reference are loaded hid the very instructions for loading them.
    ...
    try:
        # getattr, not viewer.alignment.aln: a viewer that never loaded an
        # alignment holds None there, and the attribute access raised
        # AttributeError into the catch-all handler at the bottom, which
        # printed a traceback instead of the real cause.
        alignment = getattr(viewer, 'alignment', None)
        if alignment is None or alignment.aln is None:
            Command_Engine.print_help(viewer, "Error: Global Alignment not loaded.")
            return
        ...
        if not getattr(viewer, 'active_reference', None):
            Command_Engine.print_help(
                viewer, "Error: No Reference Set. Use 'reference <ID>' first.")
            return
        ...
        # group_labels is a list of one (usually empty) set per node here, never
        # None, so the 'is None' test never fired: 'label groups' with nothing
        # grouped wrote a workbook containing only the global statistics row.
        if forced_target == "groups" and not any(getattr(viewer, 'group_labels', None) or []):
            Command_Engine.print_help(
                viewer, "Error: No groups defined. Use the 'group' command first.")
            return
        ...
        # Decoded once and reused by both splits below; the sparse matrix was
        # otherwise walked three times over.
        global_records = list(_alignment_records(viewer.alignment.aln))
        g_min, g_max, g_avg, g_std = get_sequence_stats(global_records)
        ...
        if forced_target == "clusters":
            ...
            clusters_records = {}
            for i, record in enumerate(global_records):
        ...
        # resolved_ref_full lives on the Alignment_Manager, not on the viewer, so
        # this always fell through to the raw identifier the user typed.
        ref_display = (
        ...
        try:
            ...
            # Assigning to console_text alone reported nothing: it is a
            # DummyText stand-in here, so a failed workbook write was silent.
            Command_Engine.print_help(viewer, f"IO Error: {e}")

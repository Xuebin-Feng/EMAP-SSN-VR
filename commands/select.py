...
def print_help():
    ...
    Description:
      ...
      * IMPORTANT: The selection is a terminal-side set of nodes. The headset has
        no selection-highlight channel, so nothing changes in VR until the
        selection is acted on with color, hide, size, group or save.
    ...
    Modes:
      ...
      save [filename]             : Saves selected nodes to the Header_Lists directory
                                    that @file@ reads back (Settings: HEADER_LIST_DIR)
...
def run(viewer, args):
    ...
    if args[0].lower() == "save":
        ...
        save_dir = getattr(cfg, 'HEADER_LIST_DIR',
    ...
    # Every extra token used to overwrite the previous one, so 'select "A" "B"'
    # silently selected only "B" instead of reporting the mistake.
    if len(expr_args) > 1:
        msg = ("Error: Select accepts exactly one whitespace-free Boolean expression.\n"
               f"Received {len(expr_args)}: {' '.join(expr_args)}\n"
               "Combine them with & | ^ and remove the spaces, e.g. select \"A\"&\"B\".")
        Command_Engine.print_help(viewer, msg)
        return
    ...
    if not expr:
        msg = ("Error: No logic expression provided.\n"
               "Usage: select [MODE] <EXPRESSION>   (e.g. select add \"ATA\")")
        Command_Engine.print_help(viewer, msg)
        return

    # $sele$ used to be round-tripped through the Header_Lists directory: every
    # use wrote a _sele.txt into the user's shared list folder - clobbering any
    # real file of that name - purely to read it straight back. Command_Engine
    # now resolves $sele$ from the live selection mask instead.
